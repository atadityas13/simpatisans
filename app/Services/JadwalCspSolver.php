...
/**
 * CSP backtracking + repair — target 100% JTM terisi.
 * Hard: bentrok guru/kelas, BTQ Jumat jam 5.
 * Soft: preset, struktur JTM, max 8 jam/hari (analisa saja).
 */
final class JadwalCspSolver
{
    ...
    public static function fromSemester(int $semesterId): self
    {
        ...
        $kelasRows = Kelas::orderByRaw("FIELD(tingkat, 'VII', 'VIII', 'IX')")
            ->get(['id'])
            ->map(fn($k) => [$k->id])
            ->toArray();

        $bebanRows = $beban->map(fn($b) => [
            $b->id,
            $b->semester_id,
            $b->guru_id,
            $b->mapel_id,
        ...
        $mapelRows = $beban->pluck('mapel')->filter()->unique('id')->map(fn($m) => [
            $m->id,
            $m->nama_mapel,
        ])->values()->toArray();
    ...
    public function __construct(
        ...
        $this->targetCapacity = count($this->kelasIds) * 44;
        $this->initGrid();
    }

    public function getTargetCapacity(): int
    {
    ...
    public function solve(int $timeLimit, int $seed = 0): bool
    {
        ...
        if ($this->backtrack()) {
            ...
            $this->bestFilled = $this->targetCapacity;
            return true;
        }
        ...
        if ($this->bestFilled < $this->targetCapacity) {
        ...
        return $this->bestFilled >= $this->targetCapacity;
    }
